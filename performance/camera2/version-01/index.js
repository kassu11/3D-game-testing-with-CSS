...
const renderLoop = (currentTime, previousTime) => {
  ...
  scene.style.transformOrigin = `${position.x}px ${-position.y}px ${position.z}px`;
  scene.style.transform = `translate3d(${-position.x}px, ${position.y}px, ${-position.z}px)`;
  camera.style.transform = `translate3d(0px, 0px, var(--perspective)) rotateX(${rotation.y}deg) rotateY(${rotation.x}deg) rotateZ(${rotation.z}deg)`;
}
